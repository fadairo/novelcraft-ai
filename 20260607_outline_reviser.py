...
Return the complete expanded outline with all original content enhanced."""
    ...
    def _create_restructure_prompt(self, revision_plan: str, focus_context: str) -> str:
        ...
        return f"""Restructure this outline for better narrative flow:
...
2. **Key Story Beats:**
   ...
   - First plot point (25% in)
   - Midpoint reversal (50% in)
   - Second plot point (75% in)
   - Climax (90% in)
   - Resolution
...
3. **Pacing Improvements:**
   ...
   - Vary scene lengths
   - Ensure each chapter ends with hook

4. **Scene Reordering:**
   - Move scenes for maximum impact
   - Create better cause-effect chains
   - Improve revelation timing
   - Strengthen dramatic irony
...
Return the restructured outline with clear act breaks and improved flow."""
...
Examples:
  ...
  python outline_reviser.py glasshouse --analyze-only
    ...
    try:
        ...
        print(f"Professional formatting: {'Skipped' if args.no_format else 'Applied'}")
