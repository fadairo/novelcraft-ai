#!/usr/bin/env python3
"""
chapter_reviser.py - Selective Chapter Revision Tool (Corrected)
...
class ChapterReviser:
    ...
    def _get_model_for_task(self, task_complexity: str = "medium") -> str:
        ...
        if not self.use_cost_effective_model:
            return "claude-opus-4-20250514"  # Use latest stable model
...
Focus on meaningful improvements that enhance the chapter's role in the novel while maintaining literary quality."""
    ...
    def revise_chapters(self, revision_plans: Dict[int, str]) -> Dict[int, str]:
        ...
        for chapter_num in sorted(self.target_chapters.keys()):
            ...
            if os.path.exists(revised_path):
                ...
                # Extract just the revision plan content (remove metadata)
                plan_content = revision_plans[chapter_num]
                if "---" in plan_content:
                    # Remove the header metadata
                    parts = plan_content.split("---", 2)
                    if len(parts) > 2:
                        plan_content = parts[2].strip()
                ...
                # Verify revision actually happened
                if revised_content == self.target_chapters[chapter_num]['content']:
                    print(f"    WARNING: Revision failed - content unchanged. Retrying...")
                    revised_content = self._revise_single_chapter_retry(
                        chapter_num,
                        plan_content
                    )
    ...
    def _revise_single_chapter(self, chapter_num: int, revision_plan: str) -> str:
        ...
        tasks_match = re.search(r'## SPECIFIC TASKS(.*?)(?=##|$)', revision_plan, re.DOTALL)
        specific_tasks = tasks_match.group(1) if tasks_match else "No specific tasks found"
        
        prompt = f"""You are revising Chapter {chapter_num} of a {self.genre} novel. Follow the revision plan EXACTLY and implement ALL specified changes.

REVISION PLAN TO IMPLEMENT:
{revision_plan}

ORIGINAL CHAPTER {chapter_num} ({original_word_count} words):
{chapter['content']}

CRITICAL REQUIREMENTS:
1. You MUST implement EVERY task listed in the revision plan
2. You MUST expand the chapter to approximately {target_word_count} words (current: {original_word_count})
3. You MUST maintain the original story and characters while adding new content
4. You MUST NOT simply return the original text - make substantial additions

SPECIFIC TASKS TO COMPLETE:
{specific_tasks}

EXPANSION GUIDELINES:
- Add the bookshop philosophy browsing scene (400-500 words)
- Expand Elena's father revelation with emotional depth (300-400 words)
- Add sensory details of Prague throughout
- Include Henry's physical limitations and age
- Add the Margaret memory in the bookshop
- Expand the tunnel descent sequence
- Add Henry's moment of doubt before the safe house
- Enhance all scenes with literary quality prose
...
IMPORTANT: Return ONLY the complete revised chapter text. Do not include any commentary, notes, or explanations. The chapter must be significantly longer than the original."""
        ...
        try:
            ...
            # Debug: Check if revision actually happened
            if self.debug_mode:
                print(f"    Debug: Original length: {len(chapter['content'])}")
                print(f"    Debug: Revised length: {len(cleaned_text)}")
                print(f"    Debug: First 200 chars match: {cleaned_text[:200] == chapter['content'][:200]}")
            
            # Verify the text was actually revised
            if cleaned_text == chapter['content'] or len(cleaned_text) < len(chapter['content']):
                print(f"    Warning: Chapter not properly revised. Text unchanged or shortened.")
                return self._revise_single_chapter_retry(chapter_num, revision_plan)
    ...
    def _revise_single_chapter_retry(self, chapter_num: int, revision_plan: str) -> str:
        """Retry revision with more explicit instructions."""
        chapter = self.target_chapters[chapter_num]
        original_word_count = chapter['word_count']
        target_word_count = int(original_word_count * 1.4)
        
        print(f"    Retrying with more explicit instructions...")
...
1. BOOKSHOP PHILOSOPHY SCENE (Add 400-500 words after "Henry browsed the philosophy section systematically"):
   - Henry finding books by Kafka, Kundera, and Havel that mirror his situation
   - A memory of visiting this shop with Margaret in 1978
   - Slower, more visceral discovery of the photographs
   - Internal dialogue about the philosophy of betrayal

2. ELENA'S FATHER REVELATION (Add 300-400 words after "My father was KGB, stationed here in the eighties"):
   - Elena's hands trembling as she speaks
   - Elena showing a photograph of her father
   - Henry recognizing the man from Operation Glasshouse
   - Parallel between their searches for truth

3. TUNNEL DESCENT (Add 200-300 words after "The entrance was concealed behind a Communist-era electrical box"):
   - Sensory details: coal dust, dampness, echoes
   - Henry's arthritic struggle with the ladder
   - Memories of using these tunnels in the 1980s
   - The weight of history in the stones

4. Throughout the chapter, add:
   - Henry's physical limitations (arthritis, breathlessness)
   - More sensory details of Prague
   - Enhanced literary prose
   - Philosophical observations

The revised chapter MUST be approximately {target_word_count} words (currently {original_word_count}).
...
Return ONLY the complete revised chapter with all additions integrated smoothly into the narrative."""
        ...
        try:
            ...
            # If still unchanged, return with a warning
            if cleaned_text == chapter['content']:
                print(f"    ERROR: Revision failed completely. Returning original text.")
                return chapter['content']
...
"""
    ...
    def _clean_ai_commentary(self, text: str) -> str:
        ...
        # First check if the text starts with common AI commentary patterns
        commentary_starts = [
            "Here's the revised chapter",
            "Here is the revised chapter",
            "I'll revise Chapter",
            "I will revise Chapter",
            "Let me revise",
            "I've revised",
            "I have revised",
        ]
        ...
        for pattern in commentary_starts:
            if text.lower().startswith(pattern.lower()):
                # Find the first occurrence of "Chapter" or "#" after the commentary
                chapter_start = re.search(r'(Chapter \d+|# Chapter)', text, re.IGNORECASE)
                if chapter_start:
                    text = text[chapter_start.start():]
        
        # Remove other commentary patterns
        patterns = [
            r'\[.*?\]',
            r'Would you like.*?\?',
            r'I can .*?\.',
