...
def build_prompt_claude(text, burstiness, perplexity):
    ...
    if burstiness < 0.65:  # Increased from 0.45
        notes.append("URGENT: Vary sentence lengths drastically. Mix 3-word sentences with 100+ word rambles. Break every third paragraph mid-thought.")
    if perplexity < 0.75:  # Increased from 0.6
        notes.append("CRITICAL: Add massive unpredictability. Insert contradictions, wrong facts, interrupted thoughts. Make narrator unreliable.")

    guidance = " ".join(notes) if notes else "Text needs extreme humanization. Break all patterns."

    return f"""
Human: Revise the following chapter to sound more human-written. {guidance}

CRITICAL REQUIREMENTS:
- Characters must think about the wrong things at crucial moments
- Dialogue must never quite answer the question asked
- Include factual errors and contradictions
- Break elegant prose with mundane observations
- Make the narrator occasionally forget what they're talking about

### ORIGINAL TEXT:
{text}
"""

def revise_with_claude(text, model="claude-opus-4-20250514", max_tokens=60000):
...
class SingleChapterReviser:
    ...
    def _inject_human_chaos(self, text: str) -> str:
        """Post-process to inject additional human-like chaos."""
        import re
        import random
        ...
        for i, para in enumerate(paragraphs):
            # Randomly make some paragraphs single sentences
            if random.random() < 0.15 and len(para) > 50:
                # Find first sentence end
                match = re.search(r'[.!?]\s', para)
                if match:
                    processed.append(para[:match.end()].strip())
                    processed.append(para[match.end():].strip())
                    continue
            
            # Randomly break some paragraphs at illogical points
            if random.random() < 0.1 and len(para) > 100:
                break_point = random.randint(len(para)//3, 2*len(para)//3)
                # Find nearest space
                while break_point < len(para) and para[break_point] != ' ':
                    break_point += 1
                if break_point < len(para):
                    processed.append(para[:break_point].strip())
                    processed.append(para[break_point:].strip())
                    continue
            
            # Occasionally insert very short paragraphs
            if random.random() < 0.08:
                ...
                processed.append(random.choice(short_paras))
            
            processed.append(para)
        
        # Randomly remove some paragraph breaks to create long runs
        final_text = []
        skip_next_break = False
        
        for i, para in enumerate(processed):
            if skip_next_break and i > 0:
                final_text[-1] = final_text[-1] + " " + para
                skip_next_break = False
            else:
                final_text.append(para)
                # 10% chance to merge with next paragraph
                if random.random() < 0.1 and i < len(processed) - 1:
    ...
    def analyze_chapter(self) -> str:
        ...
        prompt = f"""You are a literary writer analyzing a chapter from your book - Chapter {self.chapter_num} for revision opportunities.
...
#             return self.current_chapter['content']
    ...
    def _revise_with_plan(self, revision_plan: str, target_words: int, original_word_count: int) -> str:
        ...
        instructions = self._load_instruction_prompt()
        print("Loaded Instructions Preview:")
        print(instructions[:500])  # Show first 500 characters

        # Enhanced revision instructions with stronger AI bypass
        enhanced_instructions = """

        ## ENHANCED LITERARY FICTION AI-TO-HUMAN REVISION INSTRUCTIONS

        ### OBJECTIVE:
        Transform AI-generated literary fiction into authentically human prose that scores below 50/100 on AI detection tools.
        ...
        **1. DESTROY ALL PREDICTABLE PATTERNS**
        - Never maintain consistent metaphorical systems
        - Break logical progression of thoughts mid-sentence
        - Start scenes at random points
        - End them before natural conclusions
        - Mix time periods without explanation

        **2. SENTENCE STRUCTURE CHAOS**
        - 40/100 fragments or single words as paragraphs
        - 20/100 run-on sentences that change topic 3+ times
        - 30/100 normal sentences but with wrong punctuation
        - 10/100 sentences that contradict themselves

        **3. AUTHENTIC HUMAN ERRORS**
        - Misremember character names occasionally
        - Get ages/dates slightly wrong
        - Forget plot points established earlier
        - Mix up who said what in dialogue
        - Change your mind about descriptions

        **4. DIALOGUE AUTHENTICITY**
        - People never answer the actual question asked
        - Interrupt themselves mid-thought
        - Trail off without finishing
        - Respond to things not said
        - Mishear and respond to wrong words

        **5. NARRATIVE INCONSISTENCY**
        - Switch narrative distance randomly
        - Change tense without pattern
        - Forget what scene you're in
        - Start describing something then abandon it
        - Include details that contradict earlier ones

        ### MANDATORY ELEMENTS PER 1000 WORDS:

        - 3-5 incomplete thoughts that trail into nothing
        - 2-3 factual errors about your own story
        - 4-6 moments where narrator seems confused
        - 1-2 scenes that serve no purpose
        - 3-4 contradictions with earlier text

        ### EMOTIONAL AUTHENTICITY RULES:

        **Characters must:**
        - Feel the wrong emotions at crucial moments
        - Think about mundane things during crises
        - Forget why they're upset mid-argument
        - Have physical reactions that don't match emotions
        - Change their minds without explanation

        **Narration must:**
        - Get bored with its own descriptions
        - Start one metaphor and switch to another
        - Include wrong literary references
        - Misquote things confidently
        - Sound like someone writing at 3am

        ### PARAGRAPH STRUCTURE:
        - Never more than 2 paragraphs of similar length in a row
        - Include at least one 200+ word paragraph per chapter
        - Include at least five 1-3 word paragraphs
        - Break paragraphs at illogical points
        - Sometimes forget to paragraph at all

        ### SPECIFIC TECHNIQUES:

        **Memory and Time:**
        - "Was it Tuesday? Wednesday. No, Tuesday."
        - "She was thirty-seven. Thirty-nine. Actually—"
        - "This happened before the thing I just described. Or after."
        - "I already told this part, didn't I?"

        **Interrupting Beautiful Prose:**
        - Start poetic description, remember something mundane
        - Build to emotional climax, get distracted
        - Create metaphor, realize it's wrong, try another
        - Write beautiful sentence, immediately contradict it

        **Authentic Uncertainty:**
        - "I think that's what happened"
        - "Or maybe it was different"
        - "Actually, forget what I just said"
        - "This might be someone else's story"
        ...
        ### HUMAN WRITING MARKERS TO ADD:
        - Typos that spellcheck wouldn't catch (their/there used correctly but wrongly for context)
        - Grammar that's technically correct but sounds wrong
        - Repeated words because you forgot you used them
        - Starting sentences with "And" or "But" too much
        - Overusing certain phrases unconsciously

        ### SUCCESS METRICS:
        The revised text should feel like:
        - A first draft written across multiple moods
        - Someone telling a story while distracted
        - Notes that were meant to be fixed later
        - A story fighting against its own telling
        - Writing that forgot it was supposed to be good

        Remember: The goal is fiction that captures authentic human inconsistency, distraction, and imperfection while maintaining the story's emotional core.
        """
        
        prompt = f"""You are a literary writer. Revise this chapter by implementing EVERY task in the revision plan below.
...
Return ONLY the complete revised chapter text."""
        ...
        try:
            ...
            # Apply post-revision chaos injection
            return self._inject_human_chaos(self._clean_ai_commentary(full_response))
